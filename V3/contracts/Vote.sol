...
contract Vote{
    ...
    /// @notice 所有的普通提议
    Proposal_Common[] commonProposalQueue;
    ...
    /// @notice 每次提议收取的抵押金
    uint256 public proposalFee = 10 finney; //0.01eth
    ...
    /// @notice 一个通用提议的结构
    struct Proposal_Common{
        uint256 index;//提议的序列号
        string proposalName; //提议的名字
        address proposer; //提议人
        uint256 approveVotes; //同意的票数
        uint256 refuseVotes; //否决的票数
        uint256 startTime; //开始的时间
        bytes execScript; //需要处理的脚本bytes
        address execContract; //contract
        bool process; //是否已经处理提议
        bool pass; //是否通过
        bool abort;//是否终止
        bool oneTicketRefuse;//是否被一票否决
    ...
    /* events */
    /// @param index 提议的序列号，proposalName 提议的名称，proposer 提议者，startTime 开始提议的时间，recipient 资金接收者，value 资金数额，timeConsuming 资金接受总耗时,detail 详情
    ...
    /// @param index 提议的序列号，proposalName 提议的名称，proposer 提议者，startTime 开始提议的时间，execScript 要处理的脚本 ,execContrac 要调用的合约脚本
    event OnApplyCommonProposal(
        uint256 index,
        string proposalName,
        address proposer,
        uint256 startTime,
    ...
    /// @param who 投票人 ,proposalType 提议的类型，index 提议的序列号,voteResult 投什么票，shares 有多少股
    event OnVote(
        address who,
        uint8 proposalType,
        uint256 index,
    ...
    /// @param index 提议的序列号，proposalType 提议的类型, pass 是否通过
    event OnProcess(
        uint256 index,
        uint8 proposalType,
        bool pass
    ...
    /// @param index 提议的序列号,proposalType 提议的类型
    event OnAbort(
        uint256 index,
        uint8 proposalType
    );
    ...
    /// @param index 提议的序列号，getMoney 获取的钱，totalMoney 还剩多少钱，getTime 领取的时间
    event OnGetMoney(
        uint256 index,
        uint256 getMoney,
        uint256 totalMoney,
    ...
    /// @param proposer 发起者,startTime 开始的时间
    ...
    /// @param proposer 提议的发起人,startTime 提议开始的时间,totalShares 提议获得的总票数,pass 提议是否通过
    ...
    function applyCommonProposal(
        string memory _name,
        address _execContract,
        bytes memory _execScript,
        string memory _detail
    )
        public
        payable
        ownShares()
    {
        //发起提议的人需要缴纳一笔钱用来奖励处理者
        require(msg.value == proposalFee, "need proposalFee");
        //记录提议
        Proposal_Common memory p = Proposal_Common({
        index : commonProposalQueue.length,
        proposalName : _name,
        proposer : msg.sender,
        approveVotes : 0,
        refuseVotes : 0,
        ...
        commonProposalQueue.push(p);
        //暴露出通知
        emit OnApplyCommonProposal(p.index,p.proposalName,p.proposer,p.startTime,p.execScript,p.execContract);
    }

    /// @notice 投票通用协议
    /// @param proposalIndex 提议的序号，result 对提议的态度
    function voteCommon(uint256 proposalIndex,uint8 result) public ownShares() {
        //提议首先要存在
        ...
        emit OnVote(msg.sender,2,proposalIndex,result,shares);
    }
    ...
    /// @notice 发起清算提议
    /// @dev 股东都可以发起清算提议
    function applyClearingProposal() public payable{
        uint256 shares = getSharesOf(msg.sender);
