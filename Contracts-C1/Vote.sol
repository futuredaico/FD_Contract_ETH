...
contract N_Vote{
    ...
    /* events */
    event OnApplyProposal(uint256 index,string proposalName,address proposer, uint256 startTime,address recipient,uint256 value,uint256 timeConsuming,string detail);//提议的序列号，提议的名称，提议者，开始提议的时间，资金接收者，资金数额，资金接受总耗时,详情
    ...
    //获取总共有多少提案
    function getLengthOfProposalQueue() view public returns(uint256){
        return proposalQueue.length;
    }

    //根据index获取提案基础信息
    function getProposalInfoByIndex(uint256 proposalIndex) view public returns(string memory , address , uint256 ,address ,uint256 ,uint256 ,string memory details) {
        Proposal memory proposal = proposalQueue[proposalIndex];
        return(proposal.proposalName, proposal.proposer,proposal.startTime,proposal.recipient,proposal.value,proposal.timeConsuming,proposal.detail);
    }
    //根据index获取提案的投票状态
    function getProposalStateByIndex(uint256 proposalIndex) view public returns(uint256,uint256,bool,bool,bool,bool) {
        Proposal memory proposal = proposalQueue[proposalIndex];
        return(proposal.approveVotes,proposal.refuseVotes,proposal.process,proposal.pass,proposal.abort,proposal.oneTicketRefuse);
    }

    //查看提案资金领取情况
    function getTapByIndex(uint256 proposalIndex) view public returns(uint256 startTime , uint256 endTime,uint256 money) {
        Proposal memory proposal = proposalQueue[proposalIndex];
    ...
    //查询拥有多少股份币
    function getSharesOf(address _address) view private returns(uint256) {
        if(_address == address(0))
            _address = msg.sender;
        return fundPool.balances(_address);
    }
    
    //申请提议
    function applyProposal(string memory _name,address _recipient,uint256 _value,uint256 _timeConsuming,string memory _detail) public ownShares(){
        ...
        emit OnApplyProposal(proposal.index,proposal.proposalName,proposal.proposer,proposal.startTime,proposal.recipient,proposal.value,proposal.timeConsuming,proposal.detail);
